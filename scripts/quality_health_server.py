#!/usr/bin/env python3
"""Servidor estático do painel de qualidade + API refresh/run-tests."""
...
class QualityHealthHandler(SimpleHTTPRequestHandler):
    repo_root: Path = Path(".")
    serve_dir: Path = Path(".")
    ...
    def log_message(self, fmt: str, *args) -> None:
        if self.path.startswith("/api/"):
            sys.stderr.write(f"[qh-api] {self.address_string()} {fmt % args}\n")
    ...
    def _handle_run_tests(self) -> None:
        ...
        self._send_json(
            200 if ok else 500,
            {"ok": ok, "action": "run-tests", "message": msg or "Testes simulados e painel atualizado"},
        )
...
def main() -> int:
    p = argparse.ArgumentParser(description="Servidor painel saúde da qualidade")
    ...
    class RootedHandler(QualityHealthHandler):
        def __init__(self, *a, **kw):
            super().__init__(*a, directory=str(serve), **kw)

    httpd = ThreadingHTTPServer(("", args.port), RootedHandler)
    print(f"Painel: http://localhost:{args.port}/quality-health/")
