#!/usr/bin/env python3
"""Compare built process-metrics JSON to HEAD, ignoring volatile fields."""
...
PATH = ROOT / "docs/meta/process-metrics/process-metrics.data.json"
...
def main() -> int:
    ...
    if built != committed:
        print(
            "ERRO: process-metrics.data.json difere do commit — "
            "rode make metrics-build e git add/commit",
            file=sys.stderr,
        )
        return 1
    print("OK: process-metrics.data.json alinhado ao HEAD")
    return 0
